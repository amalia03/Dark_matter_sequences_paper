...
##Keep the sequence entries from the fasta files
comp.seqs <- comp.group[seq(2,length(comp.group), 2)]
comp2.seqs <- comp2.group[seq(2,length(comp2.group), 2)]

#Set minimum and maximum k-length, the higher it is, the longer it will take to process. 
min.k <- args[5]
#min.k <- 5
max.k <- args[6]
#max.k <- 8
...
sapply(1:length(comp.kmr), function(x){
    plot(log(comp.kmr[[x]]$counts,10), log(comp2.kmr[[x]]$counts,10),xlab="",ylab ="", col=alpha("maroon", .2), pch=19, cex.axis=2.5, cex.lab=2))
